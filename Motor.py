...
class ServoMotor :
    ...
    def Rotate(self,angle):      # make the servo rotate to specific angle (0-180 degrees)
        if(angle<0):
            angle = 0
        elif(angle > self.MAX_ANGLE):
            angle = self.MAX_ANGLE    
        #map the angle to duty cycle and output it    
        value = self.mapAngle(angle,0,self.MAX_ANGLE,self.SERVO_MIN_DUTY,self.SERVO_MAX_DUTY)    
    ...
    def loopOnce(self):
        p.start(0)
        for dc in range(0, self.MAX_ANGLE+1, 1):   #make servo rotate from 0 to 180 deg
